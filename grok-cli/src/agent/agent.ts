...
export class Agent {
  ...
  listRecentSessions(limit = 6): SessionInfo[] {
    return this.sessionStore?.listRecentSessions(limit) ?? [];
  }

  switchSession(id: string): SessionSnapshot {
    if (!this.sessionStore || !this.workspace) {
      throw new Error("Session persistence is disabled.");
    }
    const target = this.sessionStore.getSessionById(id);
    if (!target || target.workspaceId !== this.workspace.id) {
      throw new Error(`Session "${id}" was not found in this workspace.`);
    }
  ...
  forkSession(): SessionSnapshot {
    if (!this.sessionStore || !this.workspace || !this.session) {
      throw new Error("Session persistence is disabled.");
    }
    ...
    this.endCurrentSessionHook();
    this.session = this.sessionStore.createSession(this.modelId, this.mode, this.bash.getCwd());
    if (source.title) this.sessionStore.setTitle(this.session.id, `${source.title} (fork)`);
    this.messageSeqs = sourceMessages.length > 0 ? appendMessages(this.session.id, sourceMessages) : [];

...
interface: vendor `0x303a`, usage page `0xff00`, and either Codex Micro product
`0x8360` or Creator Micro V2-compatible product `33431`/`33432`
(`isCodexMicroInterface(descriptor)` matches all three).
...
**RPC envelope** — host requests are bare JSON; incoming messages are
newline-delimited:

| Direction | Shape |
| --- | --- |
| host → device | bare ASCII-safe `{"method": string, "params": any, "id": number}` |
| device → host (event) | newline-delimited `{"method"\|"m": string, "params"\|"p": any}` |
| device → host (response) | newline-delimited `{"id"\|"i": number, "method"?: string, "result": any}` |

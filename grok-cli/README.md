...
Interactive sessions also expose an authenticated, per-process Unix socket for
accessibility tools and hardware controllers. It provides semantic submit,
interrupt, mode, approval, and session actions plus lifecycle events; unsupported
actions fail explicitly instead of injecting approximate keystrokes. See
[`docs/local-control.md`](docs/local-control.md) for discovery, protocol, and
security details. Set `GROK_CONTROL_DISABLE=1` to turn it off.

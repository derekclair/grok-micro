...
describe("encodeRpcRequest", () => {
  test("emits bare ASCII-safe JSON with method, params, id", () => {
    const encoded = encodeRpcRequest({ method: RpcMethod.firmwareVersion, id: 7 });
    const text = new TextDecoder().decode(encoded);
    expect(text.endsWith("\n")).toBe(false);
    expect(text).toBe(`{"method":"sys.version","params":null,"id":7}`);
    expect(JSON.parse(text)).toEqual({ method: "sys.version", params: null, id: 7 });
  });

  test("escapes non-ASCII code units before HID fragmentation", () => {
    const text = new TextDecoder().decode(encodeRpcRequest({ method: "x", params: "café 😃", id: 0 }));
  ...
  test("rejects missing method and invalid ids", () => {
    ...
    expect(() => encodeRpcRequest({ method: "sys.version", id: 999 })).toThrow(TypeError);
  });
});

describe("RequestIdSequence", () => {
  test("cycles through the native 0-998 range", () => {
    const sequence = new RequestIdSequence();
    expect(sequence.next()).toBe(0);
    let lastId = 0;
    for (let step = 1; step <= 998; step += 1) lastId = sequence.next();
...
describe("parseRpcMessage", () => {
  test("classifies device events, responses, unknown, and invalid lines", () => {
    ...
    expect(parseRpcMessage(`{"method":"v.oai.rad","params":{"a":0.5,"d":1}}`)).toMatchObject({
      type: "event", method: "v.oai.rad", params: { a: 0.5, d: 1 },
    });
    expect(parseRpcMessage(`{"m":"v.oai.hid","p":{"k":"AG03","act":1},"i":7}`)).toMatchObject({
      type: "event", method: "v.oai.hid", params: { k: "AG03", act: 1 },
    });
    expect(parseRpcMessage(`{"i":8,"result":"ok"}`)).toMatchObject({ type: "response", id: 8, result: "ok" });

...
import { ResponseInputTracker } from "../control/response-input";
import type { LocalControlServer } from "../control/server";
import { type ControlAction, ControlActionError } from "../control/types";
import { applyConversationScroll, reasoningDirectionDelta } from "../control/ui-actions";
import {
  DEFAULT_MODEL,
...
export function App({ agent, startupConfig, initialMessage, controlServer, onExit }: AppProps) {
  const t = dark;
  ...
  const responseInputTrackerRef = useRef(new ResponseInputTracker());
  ...
  const setMode = useCallback(
    (m: AgentMode) => {
      ...
      if (currentSessionId) controlServer?.emitEvent({ type: "mode.changed", sessionId: currentSessionId, mode: m });
    },
    [agent, mode, activePlan, controlServer],
  );
  ...
  const publishSessionCatalog = useCallback(() => {
    const currentSessionId = agent.getSessionId();
    if (!currentSessionId) return;
    const sessions = agent.listRecentSessions(6).map((session) => ({
      id: session.id,
      title: session.title,
      mode: session.mode,
      updatedAt: session.updatedAt.toISOString(),
    }));
    controlServer?.updateSessions(sessions);
    controlServer?.emitEvent({
  ...
  const announceSessionChange = useCallback(
    (previousSessionId: string, snapshot: SessionSnapshot) => {
      controlServer?.updateSession(snapshot.session.id, agent.getCwd());
      controlServer?.emitEvent({
    ...
    [agent, controlServer, publishSessionCatalog],
  );

  const resetToNewSession = useCallback(() => {
    const previousSessionId = agent.getSessionId();
    const snapshot = agent.startNewSession();
    if (snapshot) applySessionSnapshot(snapshot);
    if (controlServer && previousSessionId && snapshot) {
      announceSessionChange(previousSessionId, snapshot);
    }
  }, [agent, announceSessionChange, applySessionSnapshot, controlServer]);

  const selectSession = useCallback(
    (targetSessionId: string): SessionSnapshot => {
      const previousSessionId = agent.getSessionId();
      if (!previousSessionId) throw new ControlActionError("unavailable", "No persisted session is active.");
      let snapshot: SessionSnapshot;
      try {
        snapshot = agent.switchSession(targetSessionId);
      } catch (error) {
        throw new ControlActionError("unavailable", error instanceof Error ? error.message : String(error));
      }
      applySessionSnapshot(snapshot);
      announceSessionChange(previousSessionId, snapshot);
      return snapshot;
    },
    [agent, announceSessionChange, applySessionSnapshot],
  );

  const forkSession = useCallback((): SessionSnapshot => {
    const previousSessionId = agent.getSessionId();
    if (!previousSessionId) throw new ControlActionError("unavailable", "No persisted session is active.");
    let snapshot: SessionSnapshot;
    try {
      snapshot = agent.forkSession();
    } catch (error) {
      throw new ControlActionError("unavailable", error instanceof Error ? error.message : String(error));
    }
    applySessionSnapshot(snapshot);
  ...
  const processMessage = useCallback(
    async (text: string, displayText?: string) => {
      ...
      await coordinatorRef.current.run(async () => {
        const controlledSessionId = agent.getSessionId();
        if (controlledSessionId) controlServer?.emitEvent({ type: "turn.started", sessionId: controlledSessionId });
        ...
        if (!isStale()) {
          ...
          if (currentSessionId) {
            controlServer?.emitEvent({
              type: "turn.completed",
              sessionId: currentSessionId,
              outcome: wasInterrupted ? "interrupted" : turnHadError ? "failed" : "completed",
            });
            publishSessionCatalog();
          }
  ...
  const respondToPendingApproval = useCallback(
    (decision: "approve" | "decline"): boolean => {
      const approvalId = pendingPaymentApproval?.approvalId;
      if (!approvalId) return false;
      setPendingPaymentApproval(null);
      agent.respondToToolApproval(approvalId, decision === "approve");
      const currentSessionId = agent.getSessionId();
      if (currentSessionId) {
        controlServer?.emitEvent({
          type: "input.resolved",
      ...
      if (decision === "approve") processMessage("[Payment approved]");
      return true;
    },
    [agent, controlServer, pendingPaymentApproval, processMessage],
  );
  useEffect(
    () =>
  ...
  useEffect(() => {
    const events = responseInputTrackerRef.current.transition(showPlanPanel, agent.getSessionId());
    for (const event of events) controlServer?.emitEvent(event);
  }, [agent, controlServer, showPlanPanel]);
  ...
  const handleKey = useCallback(
    (key: KeyEvent) => {
      ...
      if (pendingPaymentApproval) {
        ...
        if (key.name === "return") {
          respondToPendingApproval(pendingPaymentApproval.selected === 0 ? "approve" : "decline");
          return;
        }
        return;
      }
  ...
  useEffect(() => {
    ...
    const handleControlAction = async (action: ControlAction): Promise<unknown> => {
      switch (action.type) {
        case "composer.submit": {
          if (blockPrompt || showPlanPanel) {
            throw new ControlActionError(
              "unavailable",
              "The composer is not active while a modal or question is open.",
            );
          }
          const submitted = Boolean(inputRef.current?.plainText.trim() || pasteBlocksRef.current.length > 0);
          handleSubmit();
          return { submitted };
        }
        case "turn.interrupt":
          if (!interruptActiveRun()) throw new ControlActionError("not_active", "No turn is currently active.");
          return { interrupted: true };
        case "mode.cycle":
          if (isProcessingRef.current)
            throw new ControlActionError("busy", "Mode cannot change during an active turn.");
          if (blockPrompt || showPlanPanel) {
            throw new ControlActionError("unavailable", "Mode cannot change while a modal or question is open.");
          }
          cycleMode();
          return { mode: MODES[(MODES.findIndex((entry) => entry.id === mode) + 1) % MODES.length].id };
        case "composer.togglePlanMode": {
          if (isProcessingRef.current)
            throw new ControlActionError("busy", "Plan mode cannot change during an active turn.");
          if (blockPrompt || showPlanPanel) {
            throw new ControlActionError("unavailable", "Plan mode cannot change while a modal or question is open.");
          }
          const nextMode: AgentMode = mode === "plan" ? "agent" : "plan";
          setMode(nextMode);
          return { mode: nextMode };
        }
        case "approval.respond":
          if (action.decision !== "approve" && action.decision !== "decline") {
            throw new ControlActionError("invalid_request", "approval.respond requires approve or decline.");
          }
          if (!respondToPendingApproval(action.decision)) {
            throw new ControlActionError("not_pending", "No approval request is pending.");
          }
          return { decision: action.decision };
        case "session.new": {
          if (isProcessingRef.current)
            throw new ControlActionError("busy", "A new session cannot start during an active turn.");
          if (blockPrompt || showPlanPanel) {
            throw new ControlActionError(
              "unavailable",
              "A new session cannot start while a modal or question is open.",
            );
          }
          resetToNewSession();
          return { sessionId: agent.getSessionId() };
        }
        case "thread.select": {
          if (isProcessingRef.current)
            throw new ControlActionError("busy", "A session cannot change during an active turn.");
          if (blockPrompt || showPlanPanel) {
            throw new ControlActionError("unavailable", "A session cannot change while a modal or question is open.");
          }
          if (typeof action.sessionId !== "string" || !action.sessionId) {
            throw new ControlActionError("invalid_request", "thread.select requires a sessionId.");
          }
          const snapshot = selectSession(action.sessionId);
          return { sessionId: snapshot.session.id };
        }
        case "thread.fork": {
          if (isProcessingRef.current)
            throw new ControlActionError("busy", "A session cannot fork during an active turn.");
          if (blockPrompt || showPlanPanel) {
            throw new ControlActionError("unavailable", "A session cannot fork while a modal or question is open.");
          }
          const snapshot = forkSession();
          return { sessionId: snapshot.session.id };
        }
        case "reasoning.adjust": {
          if (action.direction !== "increase" && action.direction !== "decrease") {
            throw new ControlActionError("invalid_request", "reasoning.adjust requires increase or decrease.");
          }
          if (isProcessingRef.current) {
            throw new ControlActionError("busy", "Reasoning effort cannot change during an active turn.");
          }
          if (getSupportedReasoningEfforts(model).length === 0) {
            throw new ControlActionError("unavailable", "The current model does not expose reasoning effort.");
          }
          adjustModelReasoningEffort(model, reasoningDirectionDelta(action.direction));
          return { model, direction: action.direction };
        }
        case "conversation.scroll": {
          if (action.direction !== "up" && action.direction !== "down" && action.direction !== "bottom") {
            throw new ControlActionError("invalid_request", "conversation.scroll requires up, down, or bottom.");
          }
          try {
            if (!applyConversationScroll(scrollRef.current, action.direction, scrollToBottom)) {
              throw new ControlActionError("unavailable", "The conversation view is not mounted.");
            }
          } catch (error) {
            if (error instanceof ControlActionError) throw error;
            throw new ControlActionError("unavailable", "The conversation could not be scrolled.");
          }
        ...
        case "sidebar.toggle":
        case "settings.open":
          throw new ControlActionError("unsupported", `Grok does not implement the exact ${action.type} action.`);
        default:
          throw new ControlActionError(
            "unsupported",
            `Grok does not implement the exact ${(action as { type?: unknown }).type ?? "unknown"} action.`,
          );
      }
    };

    return controlServer.setActionHandler(handleControlAction);
  }, [
    agent,
    adjustModelReasoningEffort,
    blockPrompt,

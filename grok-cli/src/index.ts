...
import { LocalControlServer } from "./control/server";
...
async function startInteractive(
  ...
  if (process.env.GROK_CONTROL_DISABLE !== "1" && agent.getSessionId()) {
    try {
      controlServer = new LocalControlServer({ sessionId: agent.getSessionId()!, cwd: agent.getCwd() });
      ...
      process.stderr.write(`Grok local control unavailable: ${message}\n`);
    }
  }
  ...
  const shutdown = (reason: string) => {
    ...
    if (controlServer && currentSessionId) {
      controlServer.emitEvent({ type: "session.closed", sessionId: currentSessionId, reason });
    }
    void Promise.allSettled([agent.cleanup(), controlServer?.close()]).finally(() => {
      renderer.destroy();
      process.exit(0);
    });
  };
  const onInteractiveSigterm = () => shutdown("sigterm");
  process.off("SIGTERM", exitCleanlyOnSigterm);
  process.on("SIGTERM", onInteractiveSigterm);
